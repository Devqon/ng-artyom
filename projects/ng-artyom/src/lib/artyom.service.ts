...
export class ArtyomService {
  ...
  init(config?: Config) {
    ...
    const newConfig = config ? Object.assign(this.config, config) : this.config;
  ...
  addCommand(command: {indexes: string | string[], smart?: boolean, action: (i?: number, wildcard?: string) => void}) {
    this.addCommands([command]);
  }

  addCommands(commands: {indexes: string | string[], smart?: boolean, action: (i?: number, wildcard?: string) => void}[]) {
